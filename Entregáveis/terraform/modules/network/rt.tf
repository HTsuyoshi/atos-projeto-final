...
resource "aws_route_table_association" "public_assoc_nat" {
  ...
  route_table_id = aws_route_table.public_nat_rt.id
  depends_on = [
    aws_route_table.private_rds_rt,
    aws_subnet.private_rds_subnet
  ]
}
...
resource "aws_route_table" "private_rds_rt" {
  ...
  tags = {
    Name = "Route Table RDS - ${var.project_name}"
    Type = var.module_name
  }
}

resource "aws_route_table_association" "private_assoc_rds" {
  count          = length(aws_subnet.private_rds_subnet)
  subnet_id      = aws_subnet.private_rds_subnet[count.index].id
